...
exFrame$normIsPos = NA
for (i in 1:nrow(exFrame)) {
	if (exFrame$norm1[i] > 0)
		exFrame$normIsPos[i] = TRUE
	else
		exFrame$normIsPos[i] = FALSE
}
...
exFrame$easyNormIsPos = (exFrame$norm1 > 0)
head(exFrame)
tail(exFrame)

sum(exFrame$easyNormIsPos != exFrame$normIsPos)
all(exFrame$easyNormIsPos == exFrame$normIsPos)
?any #any can also be useful.  Are any of the values in these vectors different?
any(exFrame$easyNormIsPos != exFrame$normIsPos)