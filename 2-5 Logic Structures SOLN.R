...
for(i in 1:nrow(exFrame)){
  if( exFrame$norm1[i] > 0 ){
    exFrame$norm1pos[i] = TRUE
  } else {
    exFrame$norm1pos[i] = FALSE
  }
}
...
exFrame$easyNorm1Pos = ifelse( exFrame$norm1 > 0, TRUE, FALSE )
exFrame$easyNorm1Pos == exFrame$norm1pos
all( exFrame$easyNorm1Pos == exFrame$norm1pos )
