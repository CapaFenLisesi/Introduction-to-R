...
# Make sure to add a legend and label your axes.
...
allY = c(y1,y2)
plot(c(-2,2), c(min(allY), max(allY)), type="n", xlab="x", ylab="y")
lines(x,y1, lwd=2, col="dark red")
lines(x,y2, lwd=2, col="purple")

legend(.5, -3, 
	c(expression(x^3-4*x+1), expression(7*sin(x))), #Expression let's you use with math equations
	lwd=c(2,2),
	col=c("dark red", "purple"),
	y.intersp=1.5   #This command changes the vertical spacing.
)
